...
* Each server must have DNS setup for the API, wildcard applications and nameserver (ns1) 
...
5. Edit /var/named/dynamic/1.0.10.in-addr.arpa
    ...
    NOTE: In this example IP address `10.0.1.126-131` are pointed to the
...
3. Modify the user to add the `libvirt` group to the newly created user (i.e. `kni`)
   ~~~sh
...
6. Install the following packages 
   ...
   ~~~
7. Start `firewalld`, enable the `http` service, enable port 5000.
   ~~~sh
   systemctl start firewalld
   firewall-cmd --zone=public --add-service=http --permanent
   firewall-cmd --zone=public --permanent --add-service=http --permanent
...
9. Create the `default` storage pool and start it.
   ...
   ~~~
10. Configure networking (this step can also be run from the console)
    ~~~sh
    ...
    EOS
    ~~~
11. `ssh` back into your terminal session
12. Verify the connection bridges have been properly created
    ~~~sh
    nmcli con show
    ~~~
13. Login in as the new user on the provision node
    ~~~sh
    su - kni
    ~~~
14. Copy the pull secret (`pull-secret.txt`) generated earlierand place it in the provision node new user home directory
...
## Configure the install-config and run the installer
...
repurpose the provisioning node as a worker. Prior to re-provisioning 
